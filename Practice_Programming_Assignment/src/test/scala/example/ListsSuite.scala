 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum of no numbers in a list") {
    assert(sum(List()) === 0)
  }
  ...
  test("max of numbers including repeating and negative numbers") {
    assert(max(List(3, 7, 7, -8, 2, 2, 3)) === 7)
  }

  test("max of no numbers in a list - catching an exception") {
